...
WorkBuddy 若回到官方原版主题，通常是以下两种情况之一，均属「借壳」机制的预期行为、
**不是皮肤损坏**，几秒即可恢复：

- **官方更新了该主题**：官方会新建 `theme-<key>-<updatedAt>` 缓存目录并重新下载，客户端随之
  改用新目录，旧目录里你的皮肤不再被读取。
- **客户端升级改动了主题包内的主样式文件名**：例如 5.6.2 起由 `skin.css` 改为 `skin.v2.css`。
...
- **皮肤失效有两种常见原因，均可一键恢复**（运行 `doctor --fix` 或 `redo`）：
  ① **官方更新主题**：新建 `theme-<key>-<updatedAt>` 缓存目录并重新下载，客户端改用新目录，
  旧目录里的自定义 CSS 不再被读取。
  ② **客户端升级改动主题包文件名**：如 5.6.2 起主样式由 `skin.css` 改为 `skin.v2.css`。
  两者都不是皮肤损坏，也无需重新制作。
...
- 验证环境：WorkBuddy 5.5.4 / Windows（5.5.6 复验通过 2026-09-11；2026-09-14 复验
  「官方更新致失效 → doctor --fix 恢复」全流程；2026-09-22 复验并适配 5.6.2 的
  `skin.v2.css` 新文件名）。其他平台未验证。
